...
"""Send repeated left/right PTZ pulse steps to the local IP camera."""
...
from camera_ops.client import CameraPtzClient, PtzError, clamp_speed, normalize_pan_direction


def ptz_url(server_url: str, source: str, explicit_url: str | None) -> str:
    if explicit_url:
        return explicit_url
    return f"{server_url.rstrip('/')}/{source}-ptz"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Send repeated left/right PTZ pulse steps to the local IP camera.")
    parser.add_argument("direction", help="Pulse direction: left or right.")
    ...
    parser.add_argument("--settle-ms", type=int, default=2000, help="Pause between steps in milliseconds. Defaults to 2000.")
...
def build_payload(command: str, speed: int, duration_ms: int) -> dict:
    return {
        "command": command,
        "action": "pulse",
        "speed": speed,
...
def build_timed_payloads(command: str, speed: int) -> list[dict]:
    return [
        {
            "command": command,
            "action": "start",
...
def debug_log(message: str) -> None:
    print(f"[camera_ops.pan] {message}", file=sys.stderr, flush=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        command = normalize_pan_direction(args.direction)
